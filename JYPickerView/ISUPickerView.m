...
- (instancetype)init
{
    return [self initWithFrame:CGRectMake(0.0f, 0.0f, 320.0f, 210.0f)];
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.itemSize = CGSizeMake(38.0f, 44.0f);
        ...
        _cancelButton.frame = CGRectMake(0.0f, 0.0f, 140.0f, 44.0f);
        UIImage *normal = [UIImage imageNamed:@"ISUCancelButton"];
        ...
        [_cancelButton setTitleColor:[UIColor colorWithRed:153.0f/255.0f green:153.0f/255.0f blue:153.0f/255.0f alpha:1.0f] forState:UIControlStateNormal];
        [_cancelButton setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
        [self addSubview:_cancelButton];
        
        self.confirmButton = [UIButton new];
        _confirmButton.frame = CGRectMake(0.0f, 0.0f, 140.0f, 44.0f);
        normal = [UIImage imageNamed:@"ISUConfirmButton"];
        normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(5, 5, 5, 5)];
        [_confirmButton setBackgroundImage:normal forState:UIControlStateNormal];
...
- (void)layoutSubviews
{
    ...
    _cancelButton.x = (self.width - _cancelButton.width - _confirmButton.width - 10.0f) / 2;
    _cancelButton.y = _itemNameLabel.y + _itemNameLabel.height + gap;
    
    _confirmButton.x = _cancelButton.x + _cancelButton.width + 10.0f;
    _confirmButton.y = _cancelButton.y;
}
